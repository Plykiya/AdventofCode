...
class manipulation {
  ...
  /* The multiplication version of function one */
  ...
  public function nounverb($noun, $verb) {
...
/* Input data and creating an array */
...
for ($j = 0; 0 <= $j && $j <= 99; $j++){
  for ($k = 0; 0 <= $k && $k <= 99; $k++) {
    /* Resetting loop condition and data after completion */
    $arraything->resetdata($array);
    $arraything->nounverb($j, $k);
    $i = 0;
    
    /* Same loop as before, but checks the final value each time and then breaks out of all outside loops once successful */
    while ($i != 99) {
      $i = $arraything->interpretation();
        if ($arraything->finalvalue == 19690720) {
          $nounverbthing = 100 * $arraything->inputarray[1] + $arraything->inputarray[2];
          echo "$nounverbthing \n";
          break 3;
      }
    }
