...
class position {
  ...
  public function __construct($directions) {
    ...
    Set the starting coordinate values to 0 (no purpose, is for knowledge) */
    $this->horizontal = 0;
    $this->vertical = 0;
    $this->index = 0;
    ...
    foreach ($directions as $key => $value) {
      ...
      if ($direction == 'U') {
        ...
        for ($i = $original; $i != $this->horizontal; $i--) {
          $this->coordinates[$this->index]['horizontal'] = $i;
          $this->coordinates[$this->index++]['vertical'] = $this->vertical;
        }
      } elseif ($direction == 'R') {
        $original = $this->horizontal;
...
class comparison {
  ...
  function __construct($wire1coordinates, $wire2coordinates) {
    $this->shortestdistance = 0;
    
    foreach ($wire1coordinates as $wire1point => $wire1values) {
      foreach ($wire2coordinates as $wire2point => $wire2values) {
        if ($wire1values['horizontal'] == $wire2values['horizontal'] && $wire1values['vertical'] == $wire2values['vertical']) {
          $distance = $wire1values['horizontal'] + $wire1values['vertical'];
